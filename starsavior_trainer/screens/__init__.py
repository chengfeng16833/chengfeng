...
HANDLERS: dict[Screen, DelegatingScreenHandler] = {
    ...
    # 「跳过战斗」确认弹窗(双变体): 点弹窗内蓝色「跳过战斗」。payload=按钮
    # Rect(parse 按文本区分版式); priority=1 抢在蓝键 fallback 之前。
    Screen.SKIP_BATTLE_CONFIRM: DelegatingScreenHandler(
        Screen.SKIP_BATTLE_CONFIRM,
        lambda obs, state, policy: Action(
            "click",
            obs.payload if isinstance(obs.payload, Rect) else policy.config.skip_battle_confirm_button,
            "skip-battle confirm dialog, click 跳过战斗",
        ),
        priority=1,
